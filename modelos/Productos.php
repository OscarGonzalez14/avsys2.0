...
/////LISTA EXISTENCIAS EN BODEGA

public function get_existencias(){

  $conectar= parent::conexion();
  $sql= "select p.id_producto,p.modelo, p.marca,p.medidas,p.color, e.stock,e.bodega,e.categoriaub,e.id_ingreso from producto as p inner join existencias as e on p.id_producto=e.id_producto WHERE e.stock>0 order by e.id_ingreso desc;";
  $sql=$conectar->prepare($sql);
  $sql->execute();
